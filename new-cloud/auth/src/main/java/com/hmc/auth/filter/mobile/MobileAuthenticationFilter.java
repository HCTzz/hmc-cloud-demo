 ...
 * @author HHF
 * @Description
 * @create 2020-07-05 下午 4:17
 */
public class MobileAuthenticationFilter extends AbstractAuthenticationProcessingFilter {

    //该参数可以抽取成配置，最后通过配置文件进行修改，这样作为共用组件只需要实现一个 default，具体值可以有调用者指定
    ...
    public MobileAuthenticationFilter() {
        //2019/3/15 pattern 可以抽取成配置，最后通过配置文件进行修改，这样作为共用组件只需要实现一个 default，具体值可以有调用者指定
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        ...
        if (this.postOnly && !request.getMethod().equals("POST")) {
            ...
            MobileCodeAuthenticationToken token = new MobileCodeAuthenticationToken(mobile);
